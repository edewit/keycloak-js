...
export default class Keycloak {
  ...
  /** @type {import('./keycloak-dpop.js').DPoPSignatureProvider=} */
  #dpopProvider
  ...
  init = async (initOptions = {}) => {
    ...
    if (this.useDPoP && this.dpopSigningAlgValuesSupported?.length) {
      ...
      const { DPoPSignatureProvider, BrowserSignatureAlgs } = await import('./keycloak-dpop.js')
      this.#dpopProvider = new DPoPSignatureProvider({
        ...
        strictStorage: this.useDPoP.strictStorage ?? false
      })
      await this.#dpopProvider.init()
      this.#logInfo('[KEYCLOAK] DPoP initialized')
    }
  ...
  async #setupEndpoints () {
    ...
    if (this.useDPoP) {
      const issuerUrl = this.#getIssuerUrl()
      if (!issuerUrl) {
        this.#logWarn('[KEYCLOAK] Cannot fetch OIDC metadata: issuer URL is not available')
      } else {
        ...
        try {
          ...
          this.#logWarn('[KEYCLOAK] Failed to fetch OIDC metadata for DPoP configuration: ' + (error instanceof Error ? error.message : error))
        }
      }
    }
  ...
  #validateDPoPTokenType (tokenResponse) {
    ...
    if (hasDPoPProvider && tokenType !== 'dpop') {
      ...
      this.#logWarn('[KEYCLOAK] Server returned token_type "' + tokenResponse.token_type + '" instead of "DPoP". Disabling DPoP for this session.')
  ...
  #getIssuerUrl () {
    ...
    // For Keycloak mode, use the realm URL
